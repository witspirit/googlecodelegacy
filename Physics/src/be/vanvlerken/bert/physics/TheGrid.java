...
public class TheGrid extends JComponent
{
        ...
        for (int t = 0; t < PhysicsParameters.nrOfTimeSteps; t++)
        {
            if (elementsGenerated)
            {
                ...
                if (PhysicsParameters.applyGravity)
                {
                    for (int i = 0; i < elements.length; i++)
                    {
                        elements[i].applyGravity(PhysicsParameters.gravityCte);
                    }
                }

                for (int i = 0; i < elements.length; i++)
                {
                    elements[i].applyBounds(currentWidth, currentHeight);
                }
    ...
    public void paintComponent(Graphics g)
    {
        ...
        {
            logger.debug(
                "JComponent: width=" + getWidth() + ", height=" + getHeight());
            logger.debug(
                "Insets: top="
                    + myInsets.top
                    + ", bottom="
    ...
    private void updateElements()
    {
        ...
        for (int i=0; i < elements.length; i++)
        {
            elements[i].placeWithinBounds(currentWidth, currentHeight);
        }
    ...
    private Element createNonOverlappingElement(int width, int height)
    {
        ...
        {
            logger.warn(
                "Aborted overlap avoidance, maximum allowed attempts exceeded.");
        }
        else
        {
