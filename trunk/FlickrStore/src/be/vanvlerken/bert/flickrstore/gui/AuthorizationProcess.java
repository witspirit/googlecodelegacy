...
public class AuthorizationProcess implements AuthorizationHandler
{
    public boolean requestAuthorization(URL url)
    {
        ...
        if ( reqAuth.showDialog() == RequestAuthorizationDialog.DialogState.authorize )
        {
            ...
            {
                String errorMessage = "FlickrStore was unable to open a browser. Please visit "+url.toString()+" in order to complete the authorization process.";
            ...
            if ( completeAuth.showDialog() == CompleteAuthorizationDialog.DialogState.completeAuthorize )
            {
                // We should be done
